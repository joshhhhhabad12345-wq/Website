...
local CONFIG = {
    ...
    GetKeyLink        = "https://new.pandadevelopment.net/getkey?service=externalliltyserviceidentifier",
...
local function ValidateKey(key, hwid)
    ...
    -- Build the validation URL (order per Panda Dev docs: service, hwid, key)
    local url = CONFIG.ValidateEndpoint
        .. "?service=" .. CONFIG.ServiceIdentifier
        .. "&hwid=" .. tostring(hwid)
        .. "&key=" .. tostring(key)

    -- Prefer exploit HTTP functions (request / syn.request), fallback to HttpService
    local responseBody
        ...
        if httpFunc then
            local res = httpFunc({
                ...
                Headers = {
                    ...
                    -- No Authorization header needed per Panda Dev API docs
                },
            })
            responseBody = res.Body
        else
            -- Fallback: Roblox HttpService (only works in Studio / server)
            responseBody = HttpService:GetAsync(url)
        end
    end)

    if not ok or not responseBody then
        return false, "Connection error: " .. tostring(err or "Unknown")
    end

    -- Parse JSON response from Panda Development
    local decodeOk, decodeData = pcall(HttpService.JSONDecode, HttpService, responseBody)
    if not decodeOk or type(decodeData) ~= "table" then
        return false, "Invalid server response"
    end

    -- V2_Authentication is the key field returned by Panda Development
    local auth = decodeData["V2_Authentication"]
    if auth == true or auth == "true" or auth == 1 then
        local isPremium = decodeData["Key_Information"] and decodeData["Key_Information"]["Premium_Mode"]
        local label = isPremium and "Authenticated (Premium)" or "Authenticated"
        return true, label
    else
        return false, "Key rejected by Panda Development servers"
    end
end
...
local function BuildUI(hwid, onValidate)
    ...
    GetKeyBtn.MouseButton1Click:Connect(function()
        local link = CONFIG.GetKeyLink .. "&token=" .. CONFIG.APIToken .. "&hwid=" .. hwid
