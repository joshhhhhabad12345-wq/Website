--[[
    Externallity - ONE TAP
    Advanced Combat Script with Silent Aim, ESP, and Gun Modifications
...
-- NEW: FOV Circle Size/Thickness
SilentAimModule:Slider({
    Title = "FOV Circle Thickness",
    Value = { Min = 1, Max = 10, Default = Settings.SilentAim.FOVThickness },
    Flag = "silentaim_fov_thickness",
    Locked = false,
    Callback = function(value)
        Settings.SilentAim.FOVThickness = value
    end
})

-- NEW: FOV Circle Transparency
SilentAimModule:Slider({
    Title = "FOV Transparency",
    Value = { Min = 0, Max = 1, Default = Settings.SilentAim.FOVTransparency },
    Flag = "silentaim_fov_transparency",
    Locked = false,
    Callback = function(value)
...
-- NEW: FOV Circle Color Picker
SilentAimModule:Colorpicker({
    Title = "FOV Circle Color",
    Value = Settings.SilentAim.FOVColor,
    Flag = "silentaim_fov_color",
    Locked = false,
    Callback = function(color)
        Settings.SilentAim.FOVColor = color
    end
})
...
SilentAimModule:Checkbox({
    ...
    Locked = false,
    Callback = function(state)
        Settings.SilentAim.UsePrediction = state
    end
})

-- NEW: Prediction Strength Slider
SilentAimModule:Slider({
    Title = "Prediction Strength",
    Value = { Min = 0.1, Max = 2.0, Default = Settings.SilentAim.PredictionStrength },
    Flag = "silentaim_prediction_strength",
    Locked = false,
    Callback = function(value)
        Settings.SilentAim.PredictionStrength = value
    end
})
...
-- NEW: Box Color Picker
ESPModule:Colorpicker({
    Title = "Box Color",
    Value = Settings.ESP.BoxColor,
    Flag = "esp_box_color",
    Locked = false,
    Callback = function(color)
...
-- NEW: Name Color Picker
ESPModule:Colorpicker({
    Title = "Name Color",
    Value = Settings.ESP.NameColor,
    Flag = "esp_name_color",
    Locked = false,
    Callback = function(color)
...
-- NEW: Health Bar Toggle
ESPModule:Checkbox({
    Title = "Show Health Bar",
    Flag = "esp_healthbar",
    Locked = false,
    Callback = function(state)
        Settings.ESP.ShowHealthBar = state
    end
})

-- NEW: Health Bar Color Picker
ESPModule:Colorpicker({
    Title = "Health Bar Color",
    Value = Settings.ESP.HealthBarColor,
    Flag = "esp_healthbar_color",
    Locked = false,
    Callback = function(color)
        Settings.ESP.HealthBarColor = color
    end
})
...
-- NEW: Skeleton Color Picker
ESPModule:Colorpicker({
    Title = "Skeleton Color",
    Value = Settings.ESP.SkeletonColor,
    Flag = "esp_skeleton_color",
    Locked = false,
    Callback = function(color)
        Settings.ESP.SkeletonColor = color
    end
})
...
InfoSection:Paragraph({
    Title = "Script Info",
    Desc = "Externallity - ONE TAP\nVersion: 2.0 Enhanced\n\nFeatures:\n• Silent Aim with Prediction\n• Advanced ESP System\n• Weapon Modifications\n• Anti-Shove\n• Gun Tracker\n\nCreated by Externallity Team"
})
...
-- NEW: Get predicted position based on velocity
local function getPredictedPosition(part, predictionStrength)
    if not part.Parent then return part.Position end
    
    local humanoidRootPart = part.Parent:FindFirstChild("HumanoidRootPart")
    if humanoidRootPart and humanoidRootPart.AssemblyLinearVelocity then
...
RunService.RenderStepped:Connect(function()
    ...
    -- Update FOV Circle with NEW customization
...
-- Silent Aim Hook with NEW prediction
...
WeaponManager.cast = function(origin, direction, bulletSpeed, maxDistance, ...)
    if Settings.SilentAim.Enabled and CurrentTarget then
        ...
        -- NEW: Apply prediction if enabled
        local targetPosition = CurrentTarget.Position
        if Settings.SilentAim.UsePrediction then
            targetPosition = getPredictedPosition(CurrentTarget, Settings.SilentAim.PredictionStrength)
        end
...
-- NEW: Skeleton bones configuration
local SkeletonBones = {
    {"Head", "UpperTorso"},
        ...
        if player ~= LocalPlayer then
            ...
            if not (char and hrp and head and hum) or hum.Health <= 0 then
                if ESP[player] then
                    ...
                    ESP[player].HealthBarOutline.Visible = false
                    ESP[player].HealthBar.Visible = false
                    for _, line in pairs(ESP[player].SkeletonLines) do
                        line.Visible = false
                    end
                end
                continue
            ...
            local healthBarOutline = ESP[player].HealthBarOutline
            local healthBar = ESP[player].HealthBar
            local skeletonLines = ESP[player].SkeletonLines
            ...
            if onScreen then
                ...
                if Settings.ESP.ShowHealthBar then
                    ...
                    healthBarOutline.Position = Vector2.new(barX - 1, headPos.Y - 1)
                    healthBarOutline.Visible = true

                    healthBar.Size = Vector2.new(barWidth, barHeight * healthPercent)
                    healthBar.Position = Vector2.new(barX, headPos.Y + barHeight * (1 - healthPercent))
                    healthBar.Color = Settings.ESP.HealthBarColor
                    ...
                    for i, bonePair in ipairs(SkeletonBones) do
                        ...
                        if part1 and part2 and skeletonLines[i] then
                            ...
                            if visible1 and visible2 then
                                ...
                                skeletonLines[i].Color = Settings.ESP.SkeletonColor
...
print("✅ Externallity - ONE TAP Loaded Successfully")
print("✅ All Features: Silent Aim, ESP, Weapon Mods, Anti-Shove")
print("✅ Toggle UI: Right Shift")
print("✅ NEW: FOV Customization, Prediction Aim, ESP Color Changers, Health Bars, Skeleton ESP")
